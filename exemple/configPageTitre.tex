% AVERTISSEMENT : en cas de doute, toujours vous référer à la version
% officielle, au format Word, fournie par le BDR.
% Celle-ci et les instructions sont disponibles sur l'intranet :
% https://intranet.etsmtl.ca/intradm/daa/personnel-enseignant?aliaspath=/intradm/DAA/personnel-enseignant


%%% Les tailles des boites sont fixes et ne tiennent pas compte de ce
%%% qui y est écrit. Si vous souhaitez modifier la taille d'une boite, 
%%% utilisez les commandes suivantes:
%\setHauteurBoiteOuEcrireLesReponses{1.5}
%\setHauteurBoiteCalculatrice{2.5}
%\setHauteurBoiteDocumentation{1.4}
%\setHauteurBoiteAnnexes{0.9}
%\setHauteurBoiteDirectivesParticulieres{3.2}
...
\setAnnee{2024}
\setSaison{Hiver} % Choisir parmi : Hiver, Été, Automne
\setDate{8 février} % Jour et mois (l'année est déjà spécifiée avec \setAnnee)
...
% celle de la session. Typiquement, il s'agit des examens de compensation effectués
% en janvier.
...
%\setOuEcrireLesReponsesPrecisions{Pour les graphiques, utilisez des crayons de couleurs.}
...
%% Calculatrice, indiquez si elle est autorisee ou interdite (obligatoire)
...
%% Pour afficher le nombre total de points dans la case `Total` située tout en bas.
%\setNbTotalDePoints{100}
